		...
		if(!empty($_POST['emplacement']) && !empty($_POST['lieux']) && !empty($_POST['debut']) && !empty($_POST['fin']))
		{
			...
			if ($capacitetempo <= 4) {
				if($dureesejour >= 1)
				{
					...
					$sql= "INSERT INTO reservation (lieu, type, arrive, depart, electrique, disco, activites, prix, nb_emplacement, id_utilisateurs) VALUES ('$lieux', '$emplacement', '$debut', '$fin', '$electrique', '$disco', '$activites', '$prix', '$nb_emplacement', '$id_utilisateurs')";
